...
namespace NetasBank.Common
{
    ...
    {
        private static bool OkToWriteToFile = Environment.GetEnvironmentVariable("IsWriteToFile") == "1";
        private static bool OkToWriteToDatabase = Environment.GetEnvironmentVariable("IsWriteToFile") == "0";
        public static async Task<bool> Cancel(TheMasterContext _context, CancelTransactionRequestRecord request, BankEnum bankEnum)
        {
            ...
            else if (OkToWriteToFile)
            {
                _context.WriteToFile(request);
            }
            return true;
        }

        public static async Task<bool> Pay(TheMasterContext _context, CreateTransactionDetailsRequestRecord request, BankEnum bankEnum)
        {
            if (OkToWriteToDatabase)
            {
                var transaction = await _context.Transactions.FirstOrDefaultAsync(x => x.Id == request.TransactionId && x.BankId == bankEnum);
        ...
        public static async Task<bool> Refund(TheMasterContext _context, RefundTransactionRequestRecord request, BankEnum bankEnum)
        {
            if (OkToWriteToDatabase)
            {
                var transaction = await _context.Transactions.FirstOrDefaultAsync(x => x.Id == request.TransactionId && x.BankId == bankEnum);
